...
class OperateElement:
    ...
    def operate(self, mOperate):
        res = self.findElement(mOperate)
        if res["result"]:
            return self.operate_by(mOperate)
        else:
            return res
    ...
    def operate_by(self, operate):
        try:
            ...
            elements = {
                ...
                # be.ADB_TAP_CENTER: lambda: self.adb_tap_center(operate, device),
    ...
    def adb_tap_center(self, mOperate, device):
        height = str(self.driver.get_window_size()["height"]/2)
        width = str(self.driver.get_window_size()["width"]/2)
        ...
        cmd = "adb -s " + device + " shell input tap " + width + " " + height
        print(cmd)
        os.system(cmd)
